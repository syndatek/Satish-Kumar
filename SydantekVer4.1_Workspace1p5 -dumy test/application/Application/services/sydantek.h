...
extern char Sydantek_ProcessTimerEvent(); // status will return notification filaed
...
typedef struct {
     int iIptr;             // varies from 0-Max scan buffer size. Used by Periodic event to push scan data into the buffer
     int iOptr;             // varies from 0-Max scan buffer size. Used by BLE event to take out data
     int iCount;                // tells how many Scan data points are available
