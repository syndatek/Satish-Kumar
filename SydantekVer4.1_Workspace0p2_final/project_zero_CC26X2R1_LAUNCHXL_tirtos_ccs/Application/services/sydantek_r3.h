...
#define SCAN_BUF_SIZE 70


//---------------------------------------------------------------------------
typedef struct {
    volatile char cOneScanData[27]; // contains 24 bits of status + 24 bits of data for each channel
...
typedef struct {
    volatile int iIptr;             // varies from 0-Max scan buffer size. Used by Periodic event to push scan data into the buffer
    volatile int iOptr;             // varies from 0-Max scan buffer size. Used by BLE event to take out data
    volatile int iCount;                // tells how many Scan data points are available
    strOneScan ScanData[SCAN_BUF_SIZE];
    volatile uint8_t iOverrunFlag;      // This will be set when In pointer overtakes Outpointer
